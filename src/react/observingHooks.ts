...
export function useObserving<K extends 'input' | 'result' | 'touched'>(control: AnyFormControl, key: K) {
...
export function useInput<F extends AnyFormControl>(control: F): ReturnType<F['getInput']> {
  useObserving(control, 'input');

  return control.getInput();
}

export function useResult<F extends AnyFormControl>(control: F): ReturnType<F['getResult']> {
  useObserving(control, 'result');

  return control.getResult() as any;
}

export function useTouched<F extends AnyFormControl>(control: F): ReturnType<F['getTouched']> {
