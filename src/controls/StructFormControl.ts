...
> {
  ...
  constructor(
    ...
    this.output = new BehaviorSubject<O | undefined>(this.getOutput());
  ...
  getTouched(): boolean {
    return Object.values(this.controls).some((c: AnyFormControl) =>
      c.getTouched(),
    );
  }
  ...
  startValidation() {
    const inputs = Object.values(this.controls).map(
      (c: AnyFormControl) => c.input,
    );

    const inputTask = combineLatest(inputs).subscribe(() =>
      this.input.next(this.getInput()),
    );

    const results = Object.values(this.controls).map(
      (c: AnyFormControl) => c.result,
    );

    const resultTask = combineLatest(results).subscribe(() => {
      this.result.next(this.getResult());
      this.output.next(this.getOutput());
    });

    const touchedSubjects = Object.values(this.controls).map(
      (c: AnyFormControl) => c.touched,
    );

    const touchedTask = combineLatest(touchedSubjects).subscribe(() => {
